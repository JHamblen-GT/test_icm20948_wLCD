...
{
char c = 0;
    while (_cmd.readable())  _cmd.read(&c,1); // clear buffer garbage
}
...
void uLCD_4DGL :: reset()    // Reset Screen
{
    ...
    thread_sleep_for(3000);                // wait 3s for screen to restart
...
void uLCD_4DGL :: baudrate(int speed)    // set screen baud rate
{
    ...
    _cmd.set_baud(speed);                                  // set mbed to same speed
    i=0;
